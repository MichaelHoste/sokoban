  ...
  window.PathCore = (function() {
    ...
    */
    ...
    PathCore.prototype.create_from_compressed = function(compressed_path) {
      var cmpr_moves, i, move, uncmpr_moves, _i, _len, _ref, _results;
      this.n_pushes = 0;
      this.n_moves = 0;
      this.moves = [];
      cmpr_moves = [];
      uncmpr_moves = [];
      for (i = 0, _ref = compressed_path.length - 1; 0 <= _ref ? i <= _ref : i >= _ref; 0 <= _ref ? i++ : i--) {
        cmpr_moves.push(compressed_path[i]);
      }
      uncmpr_moves = this.uncompress_path(cmpr_moves);
      _results = [];
      for (_i = 0, _len = uncmpr_moves.length; _i < _len; _i++) {
        move = uncmpr_moves[_i];
    ...
    */
    ...
    PathCore.prototype.add_displacement = function(direction) {
      if (this.is_valid_direction(direction)) {
        if (direction >= 'A' && direction <= 'Z') this.n_pushes++;
        this.n_moves++;
