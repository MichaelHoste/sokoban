    ...
    LevelCore.prototype.level_from_database = function(pack_name, level_name) {
      ...
      $('meta').each(function() {
        if ($(this).attr('content') !== 'authenticity_token') {
          return token_tag = $(this).attr('content');
        }
      });
      jqxhr = $.ajax({
        type: 'POST',
      ...
      return jqxhr.success(function(data, status, xhr) {
        ...
        for (i = 0, _ref = _this.rows_number * _this.cols_number - 1; 0 <= _ref ? i <= _ref : i >= _ref; 0 <= _ref ? i++ : i--) {
    ...
    */
    ...
    LevelCore.prototype.level_from_grid = function(grid, width, height, pack_name, level_name, copyright) {
      ...
      for (i = 0, _len = grid.length; i < _len; i++) {
        line = grid[i];
        for (j = 0, _ref2 = line.length - 1; 0 <= _ref2 ? j <= _ref2 : j >= _ref2; 0 <= _ref2 ? j++ : j--) {
